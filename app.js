...
`;
    ...
    // === MODIFIKASI: Keyboard 2 Kolom + Whitelist ===
    const keyboard = {
        reply_markup: {
            inline_keyboard: [
                [
...
bot.on('callback_query', async (query) => {
    ...
    if (['create', 'hapus', 'ubahip', 'listsub', 'deleteall', 'proxy', 'attack_mode', 'whitelist_ip'].includes(data)) {
        
        let hasPermission = false;

        // 1. Cek fitur khusus Owner (Hanya Super Admin)
        // === MODIFIKASI: Tambahkan 'whitelist_ip' ===
        if (data === 'deleteall' || data === 'attack_mode' || data === 'whitelist_ip') {
            if (String(userId) === String(owner)) {
                hasPermission = true;
            } else {
                bot.answerCallbackQuery(query.id);
...
bot.on('message', async (msg) => {
    ...
    // === BARU: Handler untuk 'whitelist_ip' (harus di atas cek 'zoneId') ===
    ...
    if (!zoneId) {
        // Cek jika userState ada tapi zoneId tidak ada (ini bisa terjadi jika whitelist_ip gagal)
        delete userState[userId];
        return;
    }
