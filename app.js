...
const dns = require('dns'); // <--- [BARU] DITAMBAHKAN UNTUK HOST TO IP
...
// === TAMBAHAN: Ambil info Akun CF ===
const CfAccountId = settings.cloudflareAccountId;
const CfApiToken = settings.cloudflareApiToken; // Token ini HARUS punya izin Account:Account Firewall Access Rules:Edit

const premiumUsersFile = 'db/premiumUsers.json';
const priceFile = 'db/price.json';
const ordersFile = 'db/orders.json';
const ownersFile = 'db/owners.json';
...
// === FUNGSI BARU: Whitelist IP (Level Akun) ===
async function whitelistIp(ip, notes) {
    if (!CfAccountId || !CfApiToken) {
        return { success: false, message: '❌ Fitur ini belum dikonfigurasi oleh Owner.\n(Account ID atau API Token Akun hilang di config.js).' };
    }
    
    const url = `https://api.cloudflare.com/client/v4/accounts/${CfAccountId}/firewall/access_rules/rules`;
    const headers = {
    ...
    try {
        await axios.post(url, data, { headers });
        return { success: true, message: `✅ IP \`${ip}\` berhasil di-whitelist (Allow) di level Akun.` };
    } catch (error) {
        console.error('WHITELIST IP ERROR:', error?.response?.data || error.message);
        let cfError = "Terjadi kesalahan";
        if (error.response && error.response.data && error.response.data.errors) {
             cfError = error.response.data.errors[0].message;
        } else {
             cfError = error.message;
        }

        if (cfError.includes('duplicate') || (error.response && error.response.data.errors[0].code === 10009)) {
             return { success: false, message: `⚠️ IP \`${ip}\` sudah ada di daftar.` };
        }
        return {
            success: false,
            message: `❌ Gagal whitelist IP.\n*${cfError}*`
        };
    }
}
// === AKHIR FUNGSI BARU ===
...
`;
    ...
    // === MODIFIKASI: Keyboard 2 Kolom + Host to IP ===
    const keyboard = {
        reply_markup: {
            inline_keyboard: [
                [
                ...
                [ // Baris Owner
                    { text: '🌐 Whitelist IP', callback_data: 'whitelist_ip' }, 
                    { text: '🔥 Hapus Semua', callback_data: 'deleteall' }
                ],
                [ // [BARU] Tombol Host To IP (GRATIS)
                    { text: '🌎 Host To IP', callback_data: 'hostip' } 
                ],
                [ 
                    { text: '🕵️ Subdomain Finder', callback_data: 'subfinder' } 
                ],
                [
                    { text: '🔌 Cek Port', callback_data: 'cekport' }
                ],
                [ // Baris Beli & Kontak
...
bot.onText(/\/delowner (\d+)/, (msg, match) => {
    ...
    saveOwners(owners);
    bot.sendMessage(chatId, `✅ User ID \`${userIdToRemove}\` berhasil dihapus dari *Owner*.`, { parse_mode: 'Markdown' });
});
...
bot.on('callback_query', async (query) => {
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [
                                ...
                                { text: 'Lanjutkan ➡️', callback_data: 'buy_prem_confirm' } 
                            ]
                        ]
                    }
                }
    ...
    // Handler Tombol "Lanjutkan" Pembelian
    ...
    // === Cek Izin ===
    // === MODIFIKASI: Tambahkan 'whitelist_ip' dan 'hostip' ===
    if (['create', 'hapus', 'ubahip', 'listsub', 'deleteall', 'proxy', 'attack_mode', 'whitelist_ip', 'hostip', 'subfinder', 'cekport',].includes(data)) {
        
        let hasPermission = false;

        // [UPDATE] Izin GRATIS untuk HostIP, SubFinder, dan CekPort
        if (data === 'hostip' || data === 'subfinder' || data === 'cekport') {
             hasPermission = true; 
        }
        // 1. Cek fitur khusus Owner (Hanya Super Admin)
        // === MODIFIKASI: Tambahkan 'whitelist_ip' ===
        else if (data === 'deleteall' || data === 'attack_mode' || data === 'whitelist_ip') {
            if (String(userId) === String(owner)) {
                hasPermission = true;
            } else {
                bot.answerCallbackQuery(query.id);
            ...
            // 2. Fitur lain untuk Premium
            if (isUserPremium(userId)) {
                hasPermission = true;
            } else {
                bot.answerCallbackQuery(query.id); 
                return bot.sendMessage(chatId, '❌ Fitur ini hanya untuk *pengguna Premium*.\nStatus Anda tidak aktif. Silakan /buyprem untuk membeli.', {
                    parse_mode: 'Markdown',
                    reply_markup: {
        ...
        if (hasPermission) {
            ...
            // === BARU: Handler HOST TO IP (Tidak perlu pilih domain) ===
            if (data === 'hostip') {
                userState[userId] = { step: 'hostip' }; // Set state
                bot.answerCallbackQuery(query.id);
                // Minta domain ke user
                return bot.editMessageText('✍️ Silakan masukkan host atau domain yang ingin dikonversi ke IP:\n\nContoh: `google.com`', {
                    chat_id: chatId,
                    message_id: msgId,
                    parse_mode: 'Markdown',
                    reply_markup: {
            ...
            // === AKHIR HANDLER HOST TO IP ===
            
            // !!! TAMBAHKAN LOGIKA SUBFINDER DI SINI !!!
            if (data === 'subfinder') {
                userState[userId] = { step: 'subfinder' };
                bot.answerCallbackQuery(query.id);
                
                return bot.editMessageText('🕵️ *Subdomain Finder*\n\nSilakan masukkan domain utama yang ingin discan (OSINT):\n\nContoh: `google.com`', {
                    chat_id: chatId,
                    message_id: msgId,
                    parse_mode: 'Markdown',
                    reply_markup: {
            ...
            // !!! TAMBAHKAN LOGIKA CEKPORT DI SINI !!!
            if (data === 'cekport') {
                userState[userId] = { step: 'cekport' };
                bot.answerCallbackQuery(query.id);
                
                return bot.editMessageText('🔌 *Port Scanner*\n\nSilakan masukkan IP Address atau Domain yang ingin dicek port-nya:\n\nContoh: `1.1.1.1` atau `google.com`', {
                    chat_id: chatId,
                    message_id: msgId,
                    parse_mode: 'Markdown',
                    reply_markup: {
            ...
            // === BARU: Handler 'whitelist_ip' (tidak perlu pilih domain) ===
            if (data === 'whitelist_ip') {
                userState[userId] = { step: 'whitelist_ip' }; // Set state
                bot.answerCallbackQuery(query.id);
                // Minta IP ke user
                return bot.editMessageText('✍️ Kirim IP Address Anda yang ingin di-whitelist (Allow):\n\nContoh: `1.2.3.4`', {
                    chat_id: chatId,
                    message_id: msgId,
                    parse_mode: 'Markdown',
                    reply_markup: {
            ...
            // Untuk fitur lain, minta pilih domain
...
bot.on('message', async (msg) => {
    ...
    // === BARU: Handler untuk 'whitelist_ip' ===
    if (step === 'whitelist_ip') {
        if (String(userId) !== String(owner)) {
        ...
        if (!ipRegex.test(ip)) {
            return bot.sendMessage(chatId, '⚠️ Format IP tidak valid. Kirim IP saja, contoh: `1.2.3.4`', {
                parse_mode: 'Markdown',
                ...backToMenuKeyboard
            });
        }
        ...
        const res = await whitelistIp(ip, `Added by Bot @${botUsername}`);
    ...
    // === BARU: Handler HOST TO IP (GRATIS) ===
    if (step === 'hostip') {
        const cleanDomain = text.replace(/^(https?:\/\/)/, '').replace(/\/$/, '');
        
        dns.lookup(cleanDomain, (err, address) => {
            let replyText;
            if (err) {
                if (err.code === 'ENOTFOUND') {
                     replyText = `❌ *Host Tidak Ditemukan*\n\nHost: \`${cleanDomain}\`\nStatus: Domain tidak ditemukan atau sedang nonaktif.`;
                } else {
                     replyText = `❌ *Terjadi Kesalahan*\n\nHost: \`${cleanDomain}\`\nKesalahan: ${err.message}`;
                }
            } else {
                replyText = `🌐 *Host To IP Result*\n\nHost: \`${cleanDomain}\`\nIP Address: \`${address}\``;
            }
  ...
  // === HANDLER SUBDOMAIN FINDER (FULL CHECK ALL) ===
    if (step === 'subfinder') {
        const targetDomain = text.replace(/^(https?:\/\/)/, '').replace(/\/$/, '');

        if (!targetDomain.includes('.')) {
             return bot.sendMessage(chatId, '⚠️ Format domain tidak valid. Contoh: `example.com`', {
                ...backToMenuKeyboard
            });
        }

        // Edit pesan loading
        const loadingMsg = await bot.sendMessage(chatId, `🔍 *Menganalisa ${targetDomain}...*\n\nBot sedang mengecek IP untuk SEMUA subdomain yang ditemukan.\n_Mohon bersabar, proses ini memakan waktu..._`, { parse_mode: 'Markdown' });

        try {
            // 1. Ambil Data Nama dari crt.sh
            const response = await axios.get(`https://crt.sh/?q=%.${targetDomain}&output=json`, { timeout: 45000 }); // Naikkan timeout axios
            const data = response.data;

            if (!data || data.length === 0) {
                 bot.deleteMessage(chatId, loadingMsg.message_id).catch(()=>{});
                 bot.sendMessage(chatId, `❌ Tidak ditemukan subdomain publik untuk *${targetDomain}*.`, { 
            ...
            let rawSubdomains = data
                .map(entry => entry.name_value)
                .join('\n')
                .split('\n');

            rawSubdomains = [...new Set(rawSubdomains)]
                .filter(sub => !sub.includes('*') && sub.includes(targetDomain))
                .sort();

            const totalFound = rawSubdomains.length;

            // 3. PROSES CEK IP (SEMUA TANPA LIMIT)
            // Kita gunakan Promise.all untuk mengecek semuanya sekaligus
            // Hati-hati: Jika subdomain > 500, mungkin agak lag
            
            const resolvePromises = rawSubdomains.map(async (sub) => {
                try {
                    const { address } = await dns.promises.lookup(sub);
                    return `${sub} => ${address}`;
                } catch (e) {
                    return `${sub} => [OFFLINE/MATI]`;
                }
            });

            // Tunggu semua proses pengecekan selesai
            const finalResults = await Promise.all(resolvePromises);
            
            bot.deleteMessage(chatId, loadingMsg.message_id).catch(()=>{});

            // 4. Output (File TXT atau Pesan)
            if (totalFound > 30) { 
                const fileContent = `HASIL SCAN SUBDOMAIN: ${targetDomain}\nTotal: ${totalFound}\n\n` + finalResults.join('\n');
                const fileBuffer = Buffer.from(fileContent, 'utf8');

                await bot.sendMessage(chatId, `✅ Selesai! Ditemukan *${totalFound}* subdomain.\nSemua IP sudah dicek.\n\n_(Dikirim sebagai file)_`, { parse_mode: 'Markdown' });
                
                await bot.sendDocument(chatId, fileBuffer, {}, {
                    filename: `${targetDomain}-full-scan.txt`,
                    contentType: 'text/plain'
                });
                
                bot.sendMessage(chatId, 'Selesai.', backToMenuKeyboard);

            } else {
                const displayList = finalResults.map(s => `• \`${s}\``).join('\n');
                bot.sendMessage(chatId, `🕵️ *Hasil Subdomain & IP*\n\nTarget: \`${targetDomain}\`\nTotal: ${totalFound}\n\n${displayList}`, {
                    parse_mode: 'Markdown',
                    ...backToMenuKeyboard
                });
            }
            ...
            console.error(error);
            bot.deleteMessage(chatId, loadingMsg.message_id).catch(()=>{});
            bot.sendMessage(chatId, `❌ Terjadi kesalahan.\nTarget mungkin terlalu banyak subdomain atau API timeout.`, {
                parse_mode: 'Markdown',
                ...backToMenuKeyboard
            });
        }
    ...
    // ... (Logika subfinder ada di atas sini) ...

    // === HANDLER CEK PORT (PORT SCANNER) ===
    if (step === 'cekport') {
        const target = text.replace(/^(https?:\/\/)/, '').replace(/\/$/, ''); // Bersihkan input

        // Daftar Port yang mau dicek (Bisa Anda tambah/kurangi sendiri)
        const portsToCheck = [
            { port: 22, service: 'SSH' },
        ...
        const loadingMsg = await bot.sendMessage(chatId, `🔌 *Scanning Ports...*\n\nTarget: \`${target}\`\n_Mohon tunggu, sedang mengetuk pintu server..._`, { parse_mode: 'Markdown' });

        // Fungsi Helper untuk scan satu port
        const checkPort = (port, host) => {
            return new Promise((resolve) => {
                const socket = new net.Socket();
                socket.setTimeout(2500); // Timeout 2.5 detik per port
                ...
                socket.on('timeout', () => {
                    socket.destroy();
                    resolve('CLOSED'); // Anggap closed jika timeout
                });
        ...
        try {
            ...
            const replyText = `🔌 *Hasil Scan Port*\n\nTarget: \`${target}\`\n\n${results.join('\n')}`;
            ...
            console.error(error);
            bot.deleteMessage(chatId, loadingMsg.message_id).catch(()=>{});
            bot.sendMessage(chatId, `❌ Gagal melakukan scanning. Pastikan host valid.`, {
                parse_mode: 'Markdown',
                ...backToMenuKeyboard
            });
        }
