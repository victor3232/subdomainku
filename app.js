...
bot.on('message', async (msg) => {
  ...
  // === HANDLER SUBDOMAIN FINDER (OSINT) - UPDATE FILE TXT ===
    if (step === 'subfinder') {
        ...
        const loadingMsg = await bot.sendMessage(chatId, `🔍 Sedang mencari subdomain untuk *${targetDomain}*...\nMetode: _Certificate Transparency Log_`, { parse_mode: 'Markdown' });

        try {
            // 2. Request Data
            const response = await axios.get(`https://crt.sh/?q=%.${targetDomain}&output=json`, { timeout: 30000 });
            ...
            // === LOGIKA BARU: TEKS vs FILE TXT ===
            if (totalFound > 50) {
                // JIKA HASIL BANYAK (>50) -> KIRIM FILE .TXT
                const fileContent = subdomains.join('\n');
                const fileBuffer = Buffer.from(fileContent, 'utf8');

                // Kirim pesan info dulu
                await bot.sendMessage(chatId, `✅ Ditemukan *${totalFound}* subdomain unik untuk \`${targetDomain}\`.\n\n_(Hasil terlalu panjang, dikirim sebagai file .txt)_`, { parse_mode: 'Markdown' });
                
                // Kirim Filenya
                await bot.sendDocument(chatId, fileBuffer, {}, {
                    filename: `${targetDomain}-subdomains.txt`,
                    contentType: 'text/plain'
                });

                // Kirim tombol kembali (opsional, biar rapi)
                bot.sendMessage(chatId, 'Selesai.', backToMenuKeyboard);

            } else {
                // JIKA HASIL SEDIKIT (<=50) -> KIRIM TEKS BIASA
                const displayList = subdomains.map(s => `• \`${s}\``).join('\n');
                bot.sendMessage(chatId, `🕵️ *Hasil Subdomain Finder*\n\nTarget: \`${targetDomain}\`\nDitemukan: ${totalFound} subdomain\n\n${displayList}`, {
                    parse_mode: 'Markdown',
                    ...backToMenuKeyboard
                });
            }
            ...
            console.error(error);
            bot.deleteMessage(chatId, loadingMsg.message_id).catch(()=>{});
            bot.sendMessage(chatId, `❌ Gagal mengambil data (Timeout/Error).\nTarget mungkin terlalu aman atau API sedang sibuk.`, {
                parse_mode: 'Markdown',
                ...backToMenuKeyboard
            });
        }
